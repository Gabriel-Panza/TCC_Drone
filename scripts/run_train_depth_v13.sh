...
metadata="config/depth_v13_training.json"
...
selection="datasets/spatial_mapping/training_selections/v13_tree_hard_frames.json"
checkpoint="models/depth_anything_v2_metric_baylands_vits_v12.pth"
output="models/depth_anything_v2_metric_baylands_vits_v13.pth"
onnx_output="models/depth_anything_v2_metric_baylands_vits_v13_686x518_fp32.onnx"
log="logs/training/depth_v13.log"
...
python3 scripts/select_spatial_hard_frames.py \
  ...
  --exclude-run "$heldout" --exclude-run "$independent" \
  --radius-m 12 --per-run-target 4 --yaw-bin-deg 30 \
  --output "$selection"
...
train_command=(
  ...
  --epochs 4 --encoder vits --freeze-encoder --batch-size 2 --head-lr 1e-6
  --focus-repeat 1 --focus-frame-repeat 5
  --edge-multiplier 3.5 --unsafe-tail-weight 0.08
  --unsafe-overestimate-weight 0.30
  --unsafe-overestimate-tail-weight 0.20
  --unsafe-overestimate-tail-fraction 0.05
  --edge-overestimate-weight 0.30
  --unsafe-underestimate-weight 0.55
  --unsafe-underestimate-tail-weight 0.40
  --unsafe-underestimate-tail-fraction 0.05
  --edge-underestimate-weight 0.55
...
echo "4/6 Treinando Small v13 com perda bidirecional..."
mkdir -p "$(dirname "$log")"
...
echo "Log: $log"
echo "Nao execute SITL; o proximo passo e o gate offline v13."
