...
output="logs/spatial_offline_sweep/manual_v17_validation_01"
...
[[ -f "$model" ]] || { echo "Modelo v17 ausente; rode ./scripts/run_train_depth_v17.sh" >&2; exit 2; }
...
evaluate_depth "$reserved03" models/monocular_validation_v17_reserved03.json
evaluate_depth "$reserved06" models/monocular_validation_v17_reserved06.json
echo "4/7 Avaliando profundidade na run reservada 10..."
evaluate_depth "$reserved10" models/monocular_validation_v17_reserved10.json
echo "5/7 Avaliando profundidade na run independente antiga..."
evaluate_depth "$independent" models/monocular_validation_v17_independent.json
echo "6/7 Executando replay integral e gate de colisao..."
python3 scripts/run_spatial_offline_sweep.py \
  --matrix config/spatial_v17_validation_sweep.json \
