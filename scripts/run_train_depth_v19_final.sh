...
metadata="config/depth_v19_final_training.json"
...
selection="datasets/spatial_mapping/training_selections/v19_final_balanced_complete_frames.json"
checkpoint="models/depth_anything_v2_metric_baylands_vits_v18.pth"
output="models/depth_anything_v2_metric_baylands_vits_v19_final.pth"
onnx_output="models/depth_anything_v2_metric_baylands_vits_v19_final_686x518_fp32.onnx"
log="logs/training/depth_v19_final.log"
...
python3 scripts/select_spatial_hard_frames.py \
  ...
  --target=-24.7,25.1,-1.5 --target=-28.5,38.6,-1.5 \
  --target=-32.3,46.1,-1.5 --target=-36.4,54.8,-1.5 \
  --target=-40.6,59.7,-1.5 --target=-42.0,61.2,-1.5 \
  --target=-49.8,70.1,-1.5 --target=-46.6,65.4,-1.5 \
  --target=-34.3,52.9,-1.5 --target=-23.1,27.1,-1.5 \
  ...
  --radius-m 7 --per-run-target 1 --yaw-bin-deg 30 \
  --output "$selection"
...
train_command=(
  ...
  --epochs 2 --encoder vits --unfreeze-encoder-blocks 2 --batch-size 2 --head-lr 5e-7 --encoder-lr 2e-8
  --focus-repeat 1 --focus-frame-repeat 2
  --edge-multiplier 3.5 --unsafe-tail-weight 0.08
  --unsafe-overestimate-weight 0.55
  --unsafe-overestimate-tail-weight 0.50
  --unsafe-overestimate-tail-fraction 0.05
  --edge-overestimate-weight 0.55
  --unsafe-underestimate-weight 0.55
  --unsafe-underestimate-tail-weight 0.50
  --unsafe-underestimate-tail-fraction 0.05
  --edge-underestimate-weight 0.50
...
echo "4/6 Treinando tentativa final v19 com apenas os blocos 10-11 livres..."
mkdir -p "$(dirname "$log")"
...
echo "Log: $log"
echo "Nao execute SITL; o proximo passo e o gate offline v19_final."
