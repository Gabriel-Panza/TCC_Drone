...
metadata="config/depth_v15_training.json"
...
selection="datasets/spatial_mapping/training_selections/v15_critical_corridor_frames.json"
checkpoint="models/depth_anything_v2_metric_baylands_vits_v12.pth"
output="models/depth_anything_v2_metric_baylands_vits_v15.pth"
onnx_output="models/depth_anything_v2_metric_baylands_vits_v15_686x518_fp32.onnx"
log="logs/training/depth_v15.log"
...
train_command=(
  ...
  --unsafe-overestimate-weight 0.60
  --unsafe-overestimate-tail-weight 0.45
  --unsafe-overestimate-tail-fraction 0.05
  --edge-overestimate-weight 0.60
  --unsafe-underestimate-weight 0.20
  --unsafe-underestimate-tail-weight 0.15
  --unsafe-underestimate-tail-fraction 0.05
  --edge-underestimate-weight 0.20
...
echo "4/6 Treinando Small v15 com perda assimetrica contra profundidade excessiva..."
mkdir -p "$(dirname "$log")"
...
echo "Log: $log"
echo "Nao execute SITL; o proximo passo e o gate offline v15."
